...
export interface RadialScrollGalleryProps
  ...
  disabled?: boolean;
  /**
   * GSAP ScrollTrigger scroller element or string selector.
   * Defaults to window.
   */
  ...
  (
    ...
    useGSAP(
      () => {
        ...
        let resolvedScroller: Element | Window | undefined = undefined;
        if (typeof scroller === 'string') {
          const el = document.querySelector(scroller);
          if (el) resolvedScroller = el;
        } else if (scroller) {
          resolvedScroller = scroller;
        }

        const childElements = gsap.utils.toArray(containerRef.current.children);

        if (!prefersReducedMotion && childElements.length > 0) {
          gsap.fromTo(
            childElements,
            { scale: 0, autoAlpha: 0 },
            {
